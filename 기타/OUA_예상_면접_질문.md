...
<details>
  <summary>✅ 답변 개선</summary>
  
  - 초기에는 단일 서버 환경에서 `ReentrantLock`을 사용해 동시성 제어를 구현했습니다.  
    입찰 메서드 진입 시 락을 획득하고, `@Transactional`로 트랜잭션을 시작한 뒤 비즈니스 로직을 처리하는 구조였는데,  
    트랜잭션 커밋 전에 락이 해제되면 다른 요청이 동시에 접근하는 문제가 발생해 `TransactionSynchronizationManager`를 통해 커밋 이후 락을 해제하도록 보완했습니다.

  - 이후 구조를 리팩토링하며 **트랜잭션 분리 전략**을 도입했습니다.  
    먼저 락을 획득한 상태에서, 트랜잭션이 적용된 내부 메서드를 호출하여 DB 작업의 **원자성**을 보장했습니다.  
    트랜잭션이 커밋되고 flush가 완료된 이후에 락을 해제함으로써, 락이 풀린 뒤에도 안정적인 상태를 유지할 수 있도록 개선했습니다.
...
<details>
  ...
  - ReentrantLock은 애플리케이션 레벨에서 작동하기 때문에 DB 기반의 비관적 락이나 낙관적 락, 또는 Redis 같은 분산 락에 비해 성능이 빠르고 구현이 간단하다는 장점이 있습니다.
  
  - 특히 단일 인스턴스 환경에서는 불필요한 DB 접근이나 네트워크 비용 없이도 효율적인 동시성 제어가 가능합니다.
  
  - 반면, JVM 내에서만 유효하므로 다중 서버나 분산 환경에서는 락 일관성을 보장할 수 없다는 단점이 있습니다.
...
<details>
  ...
  - 동시성 처리에서 가장 중요하게 고려한 점은 상품의 최고 입찰가 상태였습니다.
  
  - 유저는 이 정보를 바탕으로 다음 입찰가를 제시하거나 기존 입찰을 수정하기 때문에, 동시에 여러 사용자가 입찰할 경우 동일한 가격으로 입찰이 처리되거나, 최고 입찰이 취소되는 시점에 정보가 일관되지 않을 수 있는 문제가 있었습니다.
  
  - 또한 상품이 낙찰되거나 삭제될 경우 관련된 모든 입찰 주문의 상태도 정확히 반영되어야 했습니다.
  
  - 이런 상황에서 데이터 충돌이나 상태 불일치를 막기 위해 동시성을 반드시 고려해야 했습니다.
...
<details>
  ...
  - 단순한 조회나 기본적인 CRUD 기능은 `JpaRepository`로 충분했지만, 사용자 검색 조건이 다양해지거나 동적 쿼리가 필요한 복잡한 조회 기능에서는 한계가 있었습니다.
  
  - QueryDSL은 **Type-Safe 동적 쿼리**를 자바 코드 기반으로 작성할 수 있고, 복잡한 조건 조합이나 다양한 필터링 로직을 명확하게 구현할 수 있어 선택했습니다.

  - 또한 컴파일 시점에 오류를 잡을 수 있어 유지보수성이 높고, 쿼리 구조를 코드 상에서 직관적으로 확인할 수 있어 가독성과 디버깅 측면에서도 유리했습니다.

  - 설정도 복잡하지 않고, Spring Data JPA와 함께 사용하기 편리하여 실무에서 확장성과 안정성을 모두 고려해 QueryDSL을 선택하게 되었습니다.
...
<details>
  <summary>✅ 답변</summary>
  - Offset 기반 페이징에서 데이터가 많아질수록 조회 성능이 저하되는 문제를 확인하고, Keyset 기반 페이징으로 개선했습니다.
  
  - 마지막 데이터의 키 값을 기준으로 조회하여 불필요한 정렬과 건너뛰기 비용을 줄여 Offset 조회를 개선했습니다.
  
  - UI도 무한 스크롤 방식으로 변경하여 자연스럽게 데이터의 마지막 키 값을 바탕으로 조회하도록 연결했습니다.
